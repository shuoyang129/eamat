...
class EAMAT(nn.Module):
    def __init__(self):
        ...
        self.video_affine2 = Projection(in_dim=configs.video_feature_dim,
                                        dim=configs.dim,
                                        drop_rate=configs.drop_rate)
    ...
    def forward(self, batch_word_vectors, batch_pos_tags, batch_txt_mask,
        ...
        batch_vis_feats1 = self.video_affine(batch_vis_feats)
        batch_vis_feats1 = batch_vis_feats1 * batch_vis_mask.unsqueeze(2)
        batch_vis_feats2 = self.video_affine2(batch_vis_feats)
        batch_vis_feats2 = batch_vis_feats2 * batch_vis_mask.unsqueeze(2)
        if self.video_position:
            batch_vis_feats1 = batch_vis_feats1 + self.v_pos_embedding(
                batch_vis_feats1)
        for i, module in enumerate(self.video_encoder):
            if i == 0:
                video_features = module(batch_vis_feats1, batch_vis_mask)
            else:
                video_features = module(video_features, batch_vis_mask)
        for i, module in enumerate(self.video_encoder2):
            if i == 0:
                video_features2 = module(batch_vis_feats2, batch_vis_mask)
            else:
