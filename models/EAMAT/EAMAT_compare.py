...
class EAMAT(nn.Module):
    def __init__(self):
        ...
        # ])
        self.video_encoder2 = nn.Sequential(*[
            MultiStepLSTMEncoder(in_dim=configs.dim,
                                 out_dim=configs.dim,
                                 num_layers=1,
                                 num_step=3,
                                 bi_direction=True,
        ...
        # self.post_attention_layer = getattr(
        #     attention, configs.self_attention)(configs.dim, configs.num_heads,
        #                                        configs.drop_rate)

        # self.post_attention_layer = TemporalContextModule(
        #     in_dim=configs.dim,
        #     out_dim=configs.dim,
    ...
    def forward(self, batch_word_vectors, batch_pos_tags, batch_txt_mask,
        ...
        for i, module in enumerate(self.video_encoder2):
            if i == 0:
                video_features2 = module(
                    batch_vis_feats) * batch_vis_mask.unsqueeze(2)
            else:
                video_features2 = module(
                    video_features2) * batch_vis_mask.unsqueeze(2)
        ...
        for i, module in enumerate(self.post_attention_layer):
            fused_action_feature = module(
                fused_action_feature) * batch_vis_mask.unsqueeze(2)
